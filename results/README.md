...
Everything in this folder was regenerated on the software stack pinned in
`environment.yml` and checked against the article. Verified on an independent
machine at those pinned versions (scikit-learn 1.9.0, XGBoost 3.4.1, NumPy
2.5.2, pandas 3.0.5), the following come out exactly as deposited:
...
- all eighteen rows of Table 6, in `calibration_scenarios.csv`.

One caveat about reading Table 6 rather than about reproducing it. Model B is
re-optimized rather than refitted: `code/06_calibration_scenarios.py` runs a
fresh search over the 224 original ligands. For XGBoost the two best candidates
of that search are separated by 0.000117 in the internal CV kappa that selects
them, yet differ by 0.008 in held-out AUC. The selection criterion cannot
resolve candidates whose held-out behaviour differs materially, so the identity
of "the" Model B champion is not a stable quantity at this budget, and the
*direction* of a Model A/B difference this small should not be read as
established. Its *magnitude* - small in every scenario - is the robust
observation. Section S10 of the Supplementary Material quantifies the same point
for the search budget as a whole.
...
`calibration_scenarios.csv` also reproduces in full, including the three XGBoost
Model B rows.
